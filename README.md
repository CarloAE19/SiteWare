...
System Admin: Full system configuration, user management, and global oversight.

Management / Approver: Access to AI analytics, high-level reporting, and Requisition Slip (RS) approvals.

Purchasing Officer: Manages the Supplier database, generates Purchase Orders (POs), and handles logistics tracking/SMS.

Warehouse In-Charge: Manages stock-in/stock-out, executes physical inventory audits, and operates the QR scanner.

Requestor (Project Engineer): Generates Material Requisitions (RS) for specific construction projects.
...
Clone this repository into your local web server directory (e.g., htdocs or www).

git clone [https://github.com/carloae19/cims.git](https://github.com/carloae19/cims.git)


Import the database layout (ensure you have the gb_inventory database created in phpMyAdmin).

Update Connection/db.php with your local database credentials.
...
Add a Web App to the project to get your firebaseConfig. Update assets/js/fcm.js and firebase-messaging-sw.js with these details.

Go to Project Settings > Cloud Messaging and generate a VAPID Key. Add this to assets/js/fcm.js.

Go to Project Settings > Service Accounts and generate a new private key.

Rename the downloaded file to firebase-key.json and place it securely inside the process/ directory.
...
Option A: Use Ngrok to tunnel your local port 80 to a secure HTTPS URL.
...
Password Hashing: Utilizing PHP's native password_hash() (Bcrypt).
...
System Developer: Angelo Carlo Pedrosa
