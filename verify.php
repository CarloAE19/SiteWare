...
<head>
    ...
    <title>Cryptographic Document Verification | GB Inventory</title>
...
<div class="container" style="max-width: 780px;">
    ...
    <div class="text-center mb-4">
        <h4 class="fw-bold text-dark mb-1 d-flex align-items-center justify-content-center gap-2">
            <i class="bi bi-shield-lock-fill text-primary"></i> CIMS Security Trust Center
        </h4>
        ...
        <div class="cert-card p-4 text-center">
            <div class="py-4">
                ...
                <p class="text-muted small mb-4">
                    The requested reference <code><?= htmlspecialchars($ref ?: 'EMPTY') ?></code> could not be found in the system registry.
                </p>
                <form action="verify" method="GET" class="d-flex justify-content-center gap-2 max-w-sm mx-auto" style="max-width: 400px;">
                    <input type="text" name="ref" class="form-control form-control-sm" placeholder="Enter PO-XXXX or WD-XXXX..." required>
    ...
    <?php elseif ($verificationStatus === 'VALID'): ?>
        ...
        <div class="cert-card">
            <div class="cert-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                <div>
                    <span class="badge bg-primary text-uppercase px-2 py-1 mb-2 fw-semibold" style="font-size: 0.7rem; letter-spacing: 0.5px;">
                        <?= ($type === 'po') ? 'Purchase Order Document' : 'Material Withdrawal Slip' ?>
                    </span>
                    <h4 class="fw-bold mb-0 text-white"><?= htmlspecialchars($document['po_no'] ?? $document['withdrawal_no']) ?></h4>
                </div>
                <div class="text-end">
                    <span class="badge status-badge-valid px-3 py-2 rounded-pill fw-bold d-inline-flex align-items-center gap-2">
            ...
            <div class="p-4 bg-white">
                ...
                <div class="alert alert-success d-flex align-items-start gap-3 rounded-3 mb-4 shadow-sm" role="alert">
                    ...
                    <div>
                        ...
                        <p class="small mb-0 text-dark">
                            This document's digital signature and SHA-256 data payload mathematically match the recorded Public Key cryptographic seal. 
                ...
                <div class="row g-3 mb-4">
                    <div class="col-sm-6">
                        ...
                        <div class="fw-bold text-dark"><?= htmlspecialchars($document['rs_no'] ?? $document['project_name'] ?? 'N/A') ?></div>
                    </div>
                    <div class="col-sm-6">
                    ...
                    <div class="col-sm-6">
                        ...
                        <div class="fw-bold text-dark">
                            ...
                            <span class="badge bg-secondary ms-1 fw-normal" style="font-size: 0.65rem;"><?= htmlspecialchars($signer['role']) ?></span>
                        </div>
                    </div>
                    <div class="col-sm-6">
                ...
                <div class="card border-0 bg-light rounded-3 p-3 mb-4">
                    <h6 class="fw-bold text-dark small text-uppercase mb-2">
                        <i class="bi bi-card-checklist text-primary me-1"></i> Sealed Item Manifest (<?= count($items) ?> item<?= count($items) > 1 ? 's' : '' ?>)
                    </h6>
                    <div class="table-responsive">
                        <table class="table table-sm table-borderless align-middle mb-0 bg-transparent" style="font-size: 0.85rem;">
                            <thead>
                            ...
                            <tbody>
                                <?php foreach ($items as $it): ?>
                                    <tr>
                                        ...
                                        <td class="fw-bold text-dark"><?= htmlspecialchars($it['item_name'] ?? $it['custom_item_name'] ?? $it['item_code']) ?></td>
                                        <td class="text-end fw-bold text-primary"><?= htmlspecialchars($it['quantity']) ?> <?= htmlspecialchars($it['unit'] ?? '') ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            </tbody>
                        </table>
                ...
                <div class="mb-2">
                    ...
                    <div class="hash-code p-2 d-flex justify-content-between align-items-center">
                        ...
                        <button class="btn btn-sm btn-link text-primary p-0 ms-2 text-decoration-none" onclick="navigator.clipboard.writeText('<?= htmlspecialchars($documentHash) ?>'); alert('SHA-256 Hash copied!');" title="Copy Hash">
            ...
            <div class="card-footer bg-light p-3 text-center border-top">
                <small class="text-muted d-block">
                    Verified securely by SiteWare CIMS Cryptographic Trust Engine &bull; <?= date('Y-m-d H:i:s') ?> UTC
                </small>
            </div>
        </div>
    ...
    <?php elseif ($verificationStatus === 'TAMPERED'): ?>
        ...
        <div class="cert-card border-danger">
            <div class="cert-header bg-danger text-white d-flex justify-content-between align-items-center">
                <div>
                    <span class="badge bg-white text-danger text-uppercase px-2 py-1 mb-2 fw-bold">Integrity Failure</span>
                    <h4 class="fw-bold mb-0"><?= htmlspecialchars($document['po_no'] ?? $document['withdrawal_no']) ?></h4>
                </div>
            ...
            <div class="p-4 bg-white">
                <div class="alert alert-danger d-flex align-items-start gap-3 rounded-3 mb-4" role="alert">
                    ...
                    <div>
                        ...
                        <p class="small mb-0 text-dark">
                            The current data in this document does not match the cryptographic signature generated at the time of signing. 
                            <strong>One or more items, quantities, prices, or approval details have been altered.</strong>
                        </p>
                    </div>
                </div>
                <div class="text-center py-3">
                    <p class="text-muted small mb-0">Please contact the System Administrator or Internal Auditor immediately.</p>
                </div>
            </div>
        </div>
    ...
    <?php else: ?>
        ...
        <div class="cert-card p-4 text-center">
            ...
            <p class="text-muted small mb-0">This document has not been sealed with an RSA-2048 cryptographic signature yet.</p>
        </div>
    <?php endif; ?>
