                    ...
                    <?php foreach ($users as $user): ?>
                        <tr>
                            ...
                            <td class="fw-bold text-dark" data-label="Full Name">
                                <span><i
                                        class="bi bi-person-circle me-2 text-muted"></i><?= htmlspecialchars($user['name']) ?></span>
                            </td>

                            <td class="text-primary fw-bold" data-label="Username">
                                @<?= htmlspecialchars($user['username']) ?></td>
                            <td data-label="Role"><span
                                    class="badge <?= $roleDisplay[$user['role']]['class'] ?? 'bg-secondary' ?> px-3 py-2 shadow-sm"><?= mb_strtoupper($roleDisplay[$user['role']]['label'] ?? 'UNKNOWN') ?></span>
                            </td>
                            <td class="text-center" data-label="Actions">

                                <button type="button" class="btn btn-sm btn-outline-primary shadow-sm me-1"
                                    data-bs-toggle="modal" data-bs-target="#userModal"
                                    onclick="openEditUserModal(<?= $user['id'] ?>, '<?= htmlspecialchars(addslashes($user['name'])) ?>', '<?= htmlspecialchars(addslashes($user['username'])) ?>', '<?= htmlspecialchars($user['role']) ?>')">
                                ...
                                <?php if ($user['id'] !== $_SESSION['user_id']): ?>
                                    <form method="POST" action="process/process.php" class="d-inline"
                                        ...
                                        <input type="hidden" name="action" value="delete_user"><input type="hidden"
