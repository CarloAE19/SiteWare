...
function initProjectPagination() {
    ...
    const mobileCardsContainer = document.getElementById('projectsMobileCards');
    if (!table && !mobileCardsContainer) return;

    // Remove old wrapper if re-initializing
    const oldWrapper = document.getElementById('projectPaginationWrapper') || document.querySelector('.project-pagination-wrapper');
    if (oldWrapper) oldWrapper.remove();

    const searchTerm = (document.getElementById('searchProjectsLive')?.value || '').toLowerCase().trim();

    // Collect and filter desktop rows
    let visibleRows = [];
    if (table) {
        ...
        visibleRows = allRows.filter(row => {
            ...
            const statusMatches = (currentProjectFilter === 'all' || status === currentProjectFilter);
            const searchMatches = (!searchTerm || row.textContent.toLowerCase().includes(searchTerm));
    ...
    // Collect and filter mobile cards
    let visibleCards = [];
    if (mobileCardsContainer) {
        const allCards = Array.from(mobileCardsContainer.querySelectorAll('.project-mobile-card'));
        ...
        if (noResultsMobile) {
            noResultsMobile.style.display = (allCards.length > 0 && visibleCards.length === 0) ? 'block' : 'none';
        }
    }

    const totalCount = Math.max(visibleRows.length, visibleCards.length);
    ...
    paginationWrapper.id = 'projectPaginationWrapper';
    paginationWrapper.className = 'd-flex flex-column flex-md-row justify-content-between align-items-center p-3 bg-white border rounded shadow-sm project-pagination-wrapper gap-3 mt-3';
    ...
    function showPage(page) {
        ...
        infoText.innerHTML = `Showing <b>${start + 1}</b> to <b>${Math.min(end, totalCount)}</b> of <b>${totalCount}</b> entries`;
